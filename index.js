...
class AutoExport {
  constructor(options) {
    this.watcher = chokidar.watch(options.dir || '.', { usePolling: true, ignored: options.ignored || /dist|index/})
    this.watcher.on('change', debounce(this.handleChange.bind(this), 1000))
                .on('unlink', debounce(this.handleDeleteFile.bind(this), 1000));
    this.cacheExportNameMap = {}
  }

  handleChange(pathname, stats) {
    // 运行环境目录下
    console.log('auto export compiling ...')
  ...
  autoWriteIndex(filepath, isDelete = false) {
    const nameMap = isDelete ? {} : this.getExportNames(filepath)
    // 清除对应记录的export
    if (isEmpty(nameMap)) {
    ...
    fs.readdir(dirName, {encoding: 'utf8', withFileTypes: true}, (err, files) => {
      ...
      if (!err) {
        ...
        if (!existIndex) {
          ...
          if (defaultImport) {
            const otherImport = isEmpty(exportNames) ? '' : `, { ${exportNames.join(', ')} }`
        ...
        console.log('auto export compiled')
      }
    })
  }
